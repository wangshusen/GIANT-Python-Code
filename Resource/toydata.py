...
def main():
    ...
    datatype = 'U8' # N denotes "non-uniform", 8 denotes the condition number of X
    sigma = 0.1 # noise intensity
    
    matX, vecY, vecW = generateDataLSR(n, d, datatype, sigma)

    #outfile = datatype + '_n=' + str(n) + '_d=' + str(d) + '.npz'
    outfile = datatype + '.npz'
    print(matX.shape)
    
    numpy.savez(outfile, X=matX, w=vecW, y=vecY, sigma=sigma)
